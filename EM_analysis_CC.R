...
as.data.frame(read.xlsx("CNPcreEPORfl_CC_myelinated_average_formatted.xlsx")) -> myelinated_area

## Normality
shapiro.test(myelinated_area[which(myelinated_area$group == "Control"),3]) # 0.4829
shapiro.test(myelinated_area[which(myelinated_area$group == "KO"),3]) # 0.3462
# Normally distributed

## Variance
leveneTest(myelinated_area$average_myelinated ~ myelinated_area$group) # 0.5698
...
#### G-Ratio analysis ####
as.data.frame(read.xlsx("CNPcreEPORfl_CC_gratio_format.xlsx")) -> gratio
...
openxlsx::write.xlsx(gratio_average,"gratio_average_CC.xlsx")
## Normality
shapiro.test(gratio_average[which(gratio_average$group == "Control"),3]) # 0.4085
shapiro.test(gratio_average[which(gratio_average$group == "KO"),3]) # 0.7824
# Normally distributed

## Variance
leveneTest(gratio_average$average_in_out_diameter ~ gratio_average$group) # 0.9874
# Homogeneous variance
...
## Normality
shapiro.test(gratio_average_inside[which(gratio_average_inside$group == "Control"),3]) # 0.01474
shapiro.test(gratio_average_inside[which(gratio_average_inside$group == "KO"),3]) # 0.8942
# Not-normally distributed

## Variance
leveneTest(gratio_average_inside$average_Inside.diameter ~ gratio_average_inside$group) # 0.5308
# Homogeneous variance

# Statistical test
wilcox.test(gratio_average_inside$average_Inside.diameter ~ gratio_average_inside$group)
# 0.6857
...
## Variance
leveneTest(gratio[,7] ~ gratio$group) # 0.03
# Heterogeneous variance
var(gratio[which(gratio$group == "Control"),7]) # 0.02938809
var(gratio[which(gratio$group == "KO"),7]) # 0.02384136
# This is fine...
...
oneway_test(all_diameters ~ group_factor,ties.method = "mid-ranks") #  p-value = 0.0004488
...
scale_fill_manual(values = c("Control" = "#D4D4D4", "KO" = "#D4D4D4")) +
  annotate("text", x = 1.5, y = y_pos, label = "0.614", parse = TRUE, size = 5) +
...
ggsave("myelinated_axons_average_CC.png", last_plot(), width = 10, height = 10, dpi = 600)
...
ggplot(df, aes(x = group, y = average_in_out_diameter)) +
  ...
  theme_minimal()+ 
  scale_fill_manual(values = c("Control" = "#D4D4D4", "KO" = "#D4D4D4")) +
  annotate("text", x = 1.5, y = y_pos, label = "0.718", parse = TRUE, size = 5) +
  annotate("segment", x = 1, xend = 2, y = y_pos2, yend = y_pos2) +
  theme(axis.title.x=element_blank(), # Suppress the X axis label
        axis.title.y=element_text(size=14,face="bold"), # Increase size of Y axis label and make it bold
        axis.text=element_text(size=12), # Increase size of axis text
...
ggsave("gratio_CC.png", last_plot(), width = 10, height = 10, dpi = 600)
...
ggsave("gratio vs diameter_CC.png", last_plot(), width = 10, height = 10, dpi = 600)

write.table(gratio,"CC_Figure5l_scatter.txt",sep = "\t",col.names = T,row.names = F,quote = F)
...
ggsave("gratio vs diameter_CC black line with text.png", last_plot(), width = 10, height = 10, dpi = 600)
...
ggsave("gratio vs diameter_CC.png black line no text.png", last_plot(), width = 10, height = 10, dpi = 600)

#### Figure 4 ####
gratio -> gratio_hist
round(gratio_hist$Inside.diameter,digits = 1) -> gratio_hist$Inside.diameter

write.table(gratio_hist,"CC_Figure5l_distribution.txt",sep = "\t",col.names = T,row.names = F,quote = F)
...
print(p)
ggsave("histogram_axon_diameter_CC.png", p, width = 10, height = 10, dpi = 600)
...
print(p)
ggsave("histogram_axon_diameter_count_CC.png", p, width = 10, height = 10, dpi = 600)
...
print(p)
ggsave("histogram_axon_diameter_count_CC black line with text.png", p, width = 10, height = 10, dpi = 600)
...
ggsave("histogram_axon_diameter_count_CC black line no text.png", p, width = 10, height = 10, dpi = 600)